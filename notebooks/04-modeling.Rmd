...
```{r}
train_set_ga <- readRDS("../data/train_set_ga.rds")
head(train_set_ga)
```

```{r}
test_set_ga <- readRDS("../data/test_set_ga.rds")
head(test_set_ga)
```
...
```{r}
dt_clf_ga <- train_model("rpart2", train_set_ga)
```
...
```{r}
dt_clf_ga <- train_model("rpart2", train_set_ga, tuneLength = 10)
```
...
```{r}
dt_metrics_ga <- evaluate_model(dt_clf_ga, "DT", test_set_ga)
```
...
```{r}
rf_clf_ga <- train_model("ranger", train_set_ga)
```
...
```{r}
rf_clf_ga <- train_model("ranger", train_set_ga, tuneLength = 10)
```
...
```{r}
rf_metrics_ga <- evaluate_model(rf_clf_ga, "RF", test_set_ga)
```
